...
export default function GoLivePanel({executor: runtimeExecutor, armed: runtimeArmed, chainId: botChainId}: {executor?: string; armed?: boolean; chainId?: number}) {
  ...
  const setSoak = useCallback(async () => { const hours = Number(soakHours); if (!Number.isInteger(hours) || hours < 1 || hours > 8760) { setMessage({tone: "bad", text: "Soak threshold must be an integer from 1 to 8760 hours."}); return; } setBusy(true); try { const response = await fetch(withChain("/api/bot/qualification", slug), {method: "POST", headers: {"content-type": "application/json"}, body: JSON.stringify({requiredHours: hours})}); const data = await response.json() as {ok?: boolean; error?: string; demo?: boolean}; if (!response.ok || !data.ok || data.demo) throw new Error(data.error || "bot rejected the soak threshold"); setMessage({tone: "good", text: `Dynamic soak threshold set to ${hours} hour${hours === 1 ? "" : "s"}.`}); void load(); } catch (error) { setMessage({tone: "bad", text: `Soak update failed: ${errText(error)}`}); } finally { setBusy(false); } }, [load, slug, soakHours]);
