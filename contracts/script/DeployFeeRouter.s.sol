...
contract DeployFeeRouter is Script {
    function run() external returns (ArrowHeadFeeRouter router) {
        address recipient = vm.envAddress("PLATFORM_FEE_RECIPIENT");
        uint256 pk = vm.envOr("DEPLOYER_PRIVATE_KEY", uint256(0));
        if (pk != 0) vm.startBroadcast(pk);
        else vm.startBroadcast();
        router = new ArrowHeadFeeRouter(recipient);
        vm.stopBroadcast();
        console2.log("ArrowHeadFeeRouter deployed at", address(router));
